...
contract ElectricityAuction {
    ...
    modifier onlyOwner() {
        require(
            msg.sender == address(electricityHub),
    ...
    modifier onlyActiveAuction() {
        require(
            auctionActive,
            "Function can only be executed when auction is active."
        );
        _;
    }
    ...
    modifier onlyEndedAuction() {
        require(
            !auctionActive,
            "Function can only be executed when auction has ended."
        );
        _;
    }

    function getPriceForThisAuction()
        public
        view
        onlyEndedAuction
        returns (uint256)
    ...
    function bid() external payable onlyActiveAuction {
        ...
        require(
            msg.value >= premium,
            "Value should at least cover the premium paid for the auction."
        );

        participatedInAuction[msg.sender] = Bid(
            payable(msg.sender),
            (msg.value - premium) / kwhOffered
        );
    ...
    function endAuction() external payable onlyOwner {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            ...
            if (bidderBid > localHighestBid) {
                ...
                bidderBid > localSecondHighestBid && bidderBid < localHighestBid
            ) {
    ...
    function collect() external payable onlyEndedAuction {
        ...
        if (isRenewable) {
            ...
            uint256 remainingBalance = address(this).balance;
            bool success = payable(address(electricityHub)).send(
                remainingBalance
            );
            require(success, "Transfer failed");
        }
    ...
    function getPremium() public view returns (uint256) {
        return
            (kwhOffered *
                contextData.priceCarbon *
                contextData.carbonIntensity) / 1000;
    }
}
